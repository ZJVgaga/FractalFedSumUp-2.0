...
class Client:
    ...
    def receive_data_from_server(self,server_data):
        ...
        self.logger.info(f"客户端 {self.cid} 服务器数据接收完成，已初始化单图片处理结果列表")
    ...
    def process(self):
        """
        客户端处理流程：先选择核心集，然后对核心集中的每一张图片进行处理。
        """
        self.logger.info(f"客户端 {self.cid} 开始处理流程...")
        
        # 1. 准备模型、选择核心集、准备VAE（封装成函数）
        vae = self.prepare_and_select_coreset()
        
        # 2. 处理核心集中的每一张图片
        self.logger.info(f"客户端 {self.cid} 开始处理核心集中的 {len(self.core_set_data['core_set_original_indices'])} 张图片...")
        
        # 初始化单图片处理结果列表
        if not hasattr(self, 'single_image_results'):
            self.single_image_results = []
        
        # 处理核心集中的每一张图片
        for image_index in self.core_set_data['core_set_original_indices']:
            # 处理单张图片
            result = self.process_single_core_image(image_index, vae)
            
            # 将结果添加到列表
            self.single_image_results.append(result)
            
            self.logger.info(f"客户端 {self.cid} 处理图片 {image_index} 完成，累计处理 {len(self.single_image_results)} 张图片")
        
        self.logger.info(f"客户端 {self.cid} 核心集图片处理完成，总共处理 {len(self.single_image_results)} 张图片")
        
        return
    
    def prepare_and_select_coreset(self):
        """
        准备模型、选择核心集、准备VAE。
        ...
        - vae: 准备好的VAE模型
        """
        self.logger.info(f"客户端 {self.cid} 开始准备模型、选择核心集、准备VAE...")
        
        # 1. 准备模型
        # 永远都是training_free的，所以不进行预训练
        # 直接使用接收到的模型
        self.local_model.eval()
        for param in self.local_model.parameters():    
            param.requires_grad = False
        self.logger.info(f"客户端 {self.cid} 本地模型准备完成。")
        
        # 2. 选择核心集
        self.logger.info(f"客户端 {self.cid} 开始基于训练好的本地模型选择核心集索引...")
        self.select_random_indices_based_on_trained_local_model(self.local_model, self.client_indices)
        self.logger.info(f"客户端 {self.cid} 核心集索引选择完成，核心集大小: {len(self.core_set_data['core_set_original_indices'])}")
        ...
        self.logger.info(f"客户端 {self.cid} VAE准备完成。")
        
        self.logger.info(f"客户端 {self.cid} 准备阶段完成")
    ...
    def send_data_to_server(self):
        ...
        # 检查是否有单图片处理结果需要发送
        if hasattr(self, 'single_image_results') and self.single_image_results:
            self.logger.info(f"客户端 {self.cid} 开始准备单图片处理结果以发送到服务器...")
            ...
            for result in self.single_image_results:
                latent_variables.append(result["latent_variable"])
                soft_labels.append(result["soft_label"])
                features.append(result["feature"])
                image_indices.append(result["image_index"])
                processing_costs.append(result["processing_cost"])
            
            # 将列表转换为张量
            if latent_variables:
                data_sent["latent_variables_tensor"] = torch.stack(latent_variables)
                data_sent["soft_labels"] = torch.stack(soft_labels)
                data_sent["features"] = torch.stack(features)
                data_sent["image_indices"] = torch.tensor(image_indices)
                data_sent["processing_costs"] = torch.tensor(processing_costs)
                data_sent["total_images_processed"] = len(latent_variables)
                
                # 计算批次平均特征列表（batch_mean_feature_list）
                # 服务器期望每个批次的平均特征，批次大小为256
                batch_size = 256
                ...
                for i in range(num_batches):
                    ...
                    end_idx = min((i + 1) * batch_size, len(features_tensor))
                    batch_features = features_tensor[start_idx:end_idx]
                    batch_mean = torch.mean(batch_features, dim=0)
                    batch_mean_feature_list.append(batch_mean)
                
                data_sent["batch_mean_feature_list"] = batch_mean_feature_list
                
                self.logger.info(f"客户端 {self.cid} 准备发送 {len(latent_variables)} 张图片的处理结果，"
                               f"分为 {num_batches} 个批次，每个批次平均特征已计算")
            else:
                self.logger.warning(f"客户端 {self.cid} 没有单图片处理结果可发送")
        else:
            self.logger.warning(f"客户端 {self.cid} 没有单图片处理结果")
